		...
		<view class="password-out-box">
			<view class="password">
				...
				<input placeholder="请输入密码" v-model="password" password="true"
				 placeholder-class="placeholder-class" class='weui-input' @blur="passwordCheck('密码')"/>
			</view>
			<view class="password">
				<text class="title">确认密码：</text>
				<input placeholder="请重新输入密码" v-model="confirmPassword" password="true"
				 placeholder-class="placeholder-class" class='weui-input' @blur="confirmPasswordCheck('确认密码')"/>
			</view>
		</view>
...
<script>
	...
	export default{
		...
		methods: {
			...
			passwordCheck() {
				const errorMsg = inputCheck('密码', 'password', this.password);
				if (errorMsg !== 'ok') {
					this.isValidate = false;
					error(errorMsg);
				}
			...
			confirmPasswordCheck() {
				const errorMsg = inputCheck('确认密码', 'password', this.confirmPassword);
				if (errorMsg !== 'ok') {
					this.isValidate = false;
					error(errorMsg);
				}
				var msg = checkPassword(this.password, this.confirmPassword)
				if(msg !== 'ok'){
					this.isValidate = false
					error(msg)
					return 

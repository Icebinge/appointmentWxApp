		...
		<view class="top-box">
			<view class="row-box" v-if="isAlreadyLogin" @click="toBaseInfo()">
				...
				<view class="name-card-box">
					...
					<text class="card-text" v-else>点击为自己添加预约卡</text>
				</view>
			...
			<view class="row-box" v-else @click="turnLogin()">
				...
				<view class="no-login-text-box">
					...
					<text class="welcome">欢迎来到预约系统</text>
				</view>
			</view>
		</view>
...
<!-- 				<view class="left-right-half" @click="toTreatRecord()">
				...
				<view class="left-right-half" @click="toAppointRecord()" v-if="userBaseInfo.power === 0">
					<image class="icon" src="/static/center/appoint-record.png"></image>
					<text class="text">管理预约</text>
				</view>
				<view class="left-right-half" @click="toAppointRecord()" v-else>
					<image class="icon" src="/static/center/appoint-record.png"></image>
					<text class="text">预约记录</text>
				</view>
			</view>
...
<script>
	...
	import {getPower} from '@/common/api/power.js'
	...
	export default {
		data() {
			return {
				...
				isGetTreatCard: false, // 是否填写信息获取了自己的预约卡
				...
				userBaseInfo: {
					...
					avatarUrl: '',
					phone: '',
					power: uni.getStorageSync('power') || 1,
				},
		...
		methods: {
			...
			toAppointRecord: function() {
				if(this.isAlreadyLogin) {
					if(this.userBaseInfo.power===0){
						uni.navigateTo({
							url:'/pagesA/pages/doctorAppointView'
						})
					}else{
						uni.navigateTo({
			...
			getUserInfo: function() {
				...
				getUserBaseInfo().then(res => {
					...
					this.userBaseInfo.power = (data.phone === '18650269018') ? 0 : 1;
					// console.log('power: ', this.userBaseInfo.power);
					// getPower(getToken()).then(res => {
